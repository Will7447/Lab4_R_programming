...
print(comparison_df)
```
Two facts can be concluded by the *comparison_df*. **First**, forward selection of covariates can improve performance in the context of the same type of regression model (linear regression in this case). **Second**, in the absence of forward selection for both, ridge regression model can do a better fit job compared with the linear one.  

As for the linear regression model with forward selection and the ridge regression model without forward selection, a conclusion cannot be drawn yet because there are two variables involved.
